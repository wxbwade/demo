...
//    }
    ...
    @GetMapping("cache/{id}")
    public DemoResponse getCache(@PathVariable(value = "id") Integer id) {
    ...
    public DemoResponse updateCache(@PathVariable(value = "id") Integer id) {
        User user = userService.getUser(id);
        if (user == null) {
            throw new NotFoundException();
        }
        user.setName("JSK");
        return new DemoResponse(userService.updateUser(user));
    }
}
